Shader "EranM/Plasma"
{
    ...
    SubShader
    {
        ...
        {
            float2 uv_MainTex;
            float3 worldPos; // Use world position to determine what color plasma is on the plane at one time
        };
        ...
        {
            const float PI = 3.14159265; // Use PI to make stuff circular
            ...
            // Diagonal
            c += sin(_Scale3 * (IN.worldPos.x * sin(t / 2.0) + IN.worldPos.z * cos(t / 3)) + t); // When mixing sin and cos you tend
                                          // to get circular type of patterns going on, because the formula for a circle involves sin
                                          // and cos
